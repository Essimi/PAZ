	    ...
	    var core = {
	      ...
	      init: function(element) {
	        ...
	        element.rowsOnLastPage =
	          element.rowsNum -
	          Math.floor(element.rowsNum / settings.itemsPerPage) *
	      ...
	      render: function(element) {
	        ...
	        element.scrollNavigation.panelMaxPos =
	          $dataPanel.width() - $rightPanel.width();

	        element.scrollNavigation.canScroll =
	          $dataPanel.width() > $rightPanel.width();
	        ...
	        if (settings.scrollToToday) {
	          var startPos =
	            Math.round(
	              (settings.startPos / 1000 - element.dateStart / 1000) / 86400
	            ) - 2;
	      ...
	      dataPanel: function(element, width) {
	        var dataPanel = $(
	          '<div class="dataPanel" style="width: ' + width + 'px;"/>'
	        );
	        ...
	        dataPanel.click(function(e) {
	          ...
	          var row = core.elementFromPoint(
	            leftpanel.offset().left + leftpanel.width() - 10,
	            e.pageY
	          );
	      ...
	      rightPanel: function(element, leftPanel) {
	        ...
	        switch (settings.scale) {
	          ...
	          case "hours":
	            ...
	            var dataPanel = core.dataPanel(
	              element,
	              range.length * tools.getCellSize()
	            );
	            ...
	            $('<div class="nav-slider" />')
	              .append(
	                $('<div class="nav-slider-left" />')
	                  .append(
	                    $('<span role="button" class="nav-link nav-page-prev"/>')
	                      .html("&lt;")
	                      .click(function() {
	                  ...
	                  .append(
	                    $('<div class="page-number"/>').append(
	                      $("<span/>").html(
	                        element.pageNum + 1 + " of " + element.pageCount
	                      )
	                    )
	                  )
	                  .append(
	      ...
	      markNow: function(element) {
	        switch (settings.scale) {
	          ...
	          case "months":
	            $(element)
	              .find(':findmonth("' + new Date().getTime() + '")')
	              .removeClass("wd")
	              .addClass("today");
	            break;
	          default:
	          ...
	          if (
	            ...
	            $.each(entry.values, function(j, day) {
	              ...
	              switch (settings.scale) {
	                // **Hourly data**
	                case "hours":
	                  var dFrom = tools.genId(
	                    tools.dateDeserialize(day.from).getTime(),
	                    element.scaleStep
	                  );
	                  var from = $(element).find("#dh-" + dFrom);

	                  var dTo = tools.genId(
	                    tools.dateDeserialize(day.to).getTime(),
	                    element.scaleStep
	                  );
	                ...
	                case "months":
	                  ...
	                  var from = $(element).find(
	                    "#dh-" + tools.genId(dtFrom.getTime())
	                  );
	                  var cFrom = from.attr("offset");
	                  var to = $(element).find(
	                    "#dh-" + tools.genId(dtTo.getTime())
	                  );
	                  var cTo = to.attr("offset");
	                  var dl = Math.round((cTo - cFrom) / tools.getCellSize()) + 1;
	      ...
	      navigateTo: function(element, val) {
	        ...
	        switch (val) {
	          ...
	          case "now":
	            ...
	            var val =
	              $dataPanel.find(".today").offset().left -
	      ...
	      navigatePage: function(element, val) {
	        if (
	          ...
	          element.pageNum + val <
	            Math.ceil(element.rowsNum / settings.itemsPerPage)
	        ) {
	          core.waitToggle(element, true, function() {
	            element.pageNum += val;
	      ...
	      zoomInOut: function(element, val) {
	        core.waitToggle(element, true, function() {
	          ...
	          if (
	            (zoomIn &&
	              $.inArray(scale, scales) <
	                $.inArray(settings.minScale, scales)) ||
	            (!zoomIn &&
	              $.inArray(scale, scales) > $.inArray(settings.maxScale, scales))
	          ) {
	            core.init(element);
	            return;
	          }
	      ...
	      scrollPanel: function(element, delta) {
	        ...
	        var _panelMargin =
	          parseInt(element.scrollNavigation.panelMargin, 10) + delta;
	        if (_panelMargin > 0) {
	          element.scrollNavigation.panelMargin = 0;
	          $(element)
	          ...
	          element.scrollNavigation.panelMargin =
	            element.scrollNavigation.panelMaxPos * -1;
	          $(element)
	            .find(".fn-gantt .dataPanel")
	            .css("margin-left", element.scrollNavigation.panelMargin + "px");
	        } else {
	      ...
	      waitToggle: function(element, show, fn) {
	        if (show) {
	          ...
	          if (!element.loader) {
	            element.loader = $(
	              '<div class="fn-gantt-loader" style="position: absolute; top: ' +
	                eo.top +
	                "px; left: " +
	                eo.left +
	                "px; width: " +
	    ...
	    var tools = {
	      ...
	      getMaxDate: function(element) {
	        ...
	        switch (settings.scale) {
	          case "hours":
	            maxDate.setHours(
	              Math.ceil(maxDate.getHours() / element.scaleStep) *
	                element.scaleStep
	            );
	      ...
	      getMinDate: function(element) {
	        ...
	        switch (settings.scale) {
	          case "hours":
	            minDate.setHours(
	              Math.floor(minDate.getHours() / element.scaleStep) *
	                element.scaleStep
	            );
	      ...
	      parseTimeRange: function(from, to, scaleStep) {
	        ...
	        do {
	          ...
	          current.setHours(
	            Math.floor(current.getHours() / scaleStep) * scaleStep
	          );
