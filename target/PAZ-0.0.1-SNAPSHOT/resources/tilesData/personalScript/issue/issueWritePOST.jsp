...
$(function () {
	...
	var message = "${status}";
	if (message == "FAIL") {
		toastr.error("실패하셨습니다. 다시 시도해주세요.");
	};
...
$('#topWorkSearch').keyup(function () {
  ...
  $("#issueSpeaker").on("click", function(){
	  $("input[name='issueName']").val("log4j관련 이슈 입니다.");
	  $("#topWorkCodeDiv").append(
		  $("<p>").attr("style", "margin-top:10px").append(
		  	$("<i>").css({"color" : "#dc3545", "margin-right" : "10px"}).addClass("fas fa-check")	  
		  ).append(" WORK00301 - 결제 구현 ")  
		  , $("<input>").attr({"type" : "hidden", "name" : "workCode"}).val("WORK00301")	  
	  );
	  $("select[name='issueTypeCode']").val("ISSUE_TYPE002").prop("selected", true);
	  $("select[name='importance']").val("IMPORTANCE001").attr("selected", true);
	  $('#compose-textarea').summernote('editor.insertText', ' log4j 관련 보안 이슈 입니다. log4j의 버전을 2.16 이상으로 업그레이드 해주세요.');
	  $("input:checkbox[name='recipient']").attr("checked", true);
  })
  
    $("#issueHelper").on("click", function(){
	  $("input[name='issueName']").val("log4j 관련 긴급 이슈 입니다.");
	  $("#topWorkCodeDiv").append(
		  $("<p>").attr("style", "margin-top:10px").append(
		  	$("<i>").css({"color" : "#dc3545", "margin-right" : "10px"}).addClass("fas fa-check")	  
		  ).append(" WORK00301 - 결제 구현 ")  
	  ...
	  $("select[name='issueTypeCode']").val("ISSUE_TYPE001").prop("selected", true);
	  $("select[name='importance']").val("IMPORTANCE001").attr("selected", true);
	  $('#compose-textarea').summernote('editor.insertText', ' log4j 임시방편으로 git repo 에서 PatternLayout 부분 수정 부탁드립니다. ');
	  $("input:checkbox[value='aa004']").attr("checked", true);
	  $("input:checkbox[value='bb005']").attr("checked", true);
	  $("input:checkbox[value='cc001']").attr("checked", true);
  })
