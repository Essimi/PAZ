...
<script>
	...
	$('.marginClass').click(function() {
		...
		$.ajax({
			...
			dataType : "json",
			url : "${cPath}/project/${project.pCode}/chat/chattingProfile.do",
...
<script>
	...
	$('#startChat').click(function() {
		...
		$.ajax({
			...
			dataType : "json",
			url : "${cPath}/project/${project.pCode}/chat/oneMakeChat.do",
			method : "POST",
			async:false,
			success : function(resp) {
...
<script>
	...
	$('#makeChatBtn').click(function() {
		...
		$.ajax({
			...
			dataType : "json",
			url : "${cPath}/project/${project.pCode}/chat/makeChat.do",
			method : "POST",
...
<script>
	...
	let deleteId = null;
	let deleteChatroomCode = null;

	$('.deleteChatroom').click(function() {
		
		deleteId = $('#myId').val();
		deleteChatroomCode = $(this).attr('id');
		
		$('#chatDeleteModal').modal();
	...
	function outChatroom() {
		...
		console.log(deleteChatroomCode);
		
		$.ajax({
			data : {
				"deleteId" : deleteId,
				"deleteChatroomCode" : deleteChatroomCode
			},
			dataType : "json",
			url : "${cPath}/project/${project.pCode}/chat/chatDelete.do",
			method : "POST",
			...
			success : function(resp) {
				...
				deleteTag = '#deleteChatTag' + deleteChatroomCode;
				$("tbody").remove(deleteTag);
